...
noob::model::mesh::mesh(aiScene* scene, aiMesh* mesh, const std::string& filepath)
{
	...
	if(mesh->HasPositions())
	{
		...
		for(int i = 0; i < num_verts; ++i)
		{
			...
			position_normal_vertex[i].m_x = mesh->mVertices[i].x;
			position_normal_vertex[i].m_y = mesh->mVertices[i].y;
			position_normal_vertex[i].m_z = mesh->mVertices[i].z;
		}
	...
	*/
	if(mesh->HasNormals()) 
	{
		...
		for(int i = 0; i < num_verts; ++i)
		{
			...
			vertices[i].m_normal = pack_four_floats_to_uint32(mesh->mNormals[i].x, mesh->mNormals[i].y, mesh->mNormals[i].z);
		}
		//	delete normals;
	}
...
void noob::model::mesh::load_textures(aiMaterial* mat, aiTextureType type, const std::string& filepath)
{
	for(size_t i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		texture = bgfx_utils::loadTexture(tex_path);
