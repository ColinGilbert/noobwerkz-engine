...
void noob::register_particles(asIScriptEngine* script_engine)
{
	...
	r = script_engine->RegisterObjectProperty("particle_system_descriptor", "vec3 wind", asOFFSET(noob::particle_system::descriptor, wind)); assert(r >= 0);
	r = script_engine->RegisterObjectProperty("particle_system_descriptor", "reflectance_handle reflect", asOFFSET(noob::particle_system::descriptor, reflect)); assert(r >= 0);
