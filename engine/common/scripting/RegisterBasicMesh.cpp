...
void noob::register_basic_mesh(asIScriptEngine* script_engine)
{
	...
	r = script_engine->RegisterObjectMethod("basic_mesh", "bool save(const string& in)", asMETHODPR(noob::basic_mesh, save, (const std::string&) const, bool), asCALL_THISCALL); assert( r >= 0 );
