...
bgfx::VertexDecl noob::drawable::position_normal_colour_vertex::ms_decl;
...
void noob::drawable::draw(uint8_t view_id, const float* transform, bgfx::ProgramHandle program_handle)
{
	...
	logger::log("Drawable drawing");
	logger::log("Setting transform");
	uint32_t cached = bgfx::setTransform(transform);
	logger::log("Setting program handle");
	bgfx::setProgram(program_handle);

	logger::log("Setting index buffer");
	bgfx::setIndexBuffer(index_buffer);

	logger::log("Setting vertex buffer");
	bgfx::setVertexBuffer(vertex_buffer);
	logger::log("Setting flags");
	bgfx::setState(flags);
	logger::log("Submitting");
	bgfx::submit(view_id);
}

noob::drawable::drawable(aiScene* scene, aiMesh* drawable, const std::string& filepath) : flags(BGFX_STATE_DEFAULT)
{
	logger::log("Creating drawable.");

	position_normal_colour_vertex::init();

	vertex_buffer.idx = bgfx::invalidHandle;
	index_buffer.idx = bgfx::invalidHandle;
	...
	vertices = new position_normal_colour_vertex[num_verts];
	...
	*/
	...
	if(drawable->HasFaces())
	{
		...
		{
			std::stringstream ss;
			ss << "Created index buffer. Handle valid? ";
			bool valid = bgfx::invalidHandle != index_buffer.idx;
			if (valid) ss << "true";
			else ss << "false";
			logger::log(ss.str());
		}
	...
	vertex_buffer = bgfx::createVertexBuffer(bgfx::makeRef(vertices, sizeof(vertices)), position_normal_colour_vertex::ms_decl);

		{
			std::stringstream ss;
			ss << "Created vertex buffer. Handle valid? ";
			bool valid = bgfx::invalidHandle != vertex_buffer.idx;
			if (valid) ss << "true";
			else ss << "false";
			logger::log(ss.str());
		}
