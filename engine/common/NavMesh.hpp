...
namespace noob
{
	class navigation
	{
		public:
			...
			navigation() noexcept(true) : navmesh(nullptr), keep_temporaries(false), heightfield(nullptr), compact_heightfield(nullptr), contours(nullptr), polymesh(nullptr), polymesh_detail(nullptr), cfg({0}), partitioning(partition_type::WATERSHED), offmesh_connections_count(0), convexes_count(0) {}
			...
			enum class poly_flag : uint16_t
		{
			...
			// At this point in time, all obstacles are boxes.
			bool add_obstacle(const noob::vec3& scales, const noob::vec3& pos, const noob::versor& orient, float height_min, float height_max, noob::navigation::poly_type) noexcept(true);

			void clear_temporaries() noexcept(true);
