...
namespace noob
{
	namespace glsl
	{
		...
		static std::string get_light(
				...
				"	float diffuse_coeff = lambertDiffuse(light_direction, world_normal);						\n"
				"	// float diffuse_coeff = orenNayarDiffuse(light_direction, view_direction, world_normal, roughness, albedo);	\n"
				"	float diffuse = (diffuse_coeff * falloff);									\n"
				"	// float specular = 0.0;											\n"
				"	float specular = blinnPhongSpecular(light_direction, view_direction, world_normal, u_shine);			\n"
				"	// float specular = cookTorranceSpecular(light_direction, view_direction, world_normal, roughness, fresnel);	\n"
				"	vec3 light_colour = (diffuse + specular) * light_rgb;								\n"
				"	return light_colour;												\n"
				"}															\n");


		static std::string lambert_diffuse(
		...
		static std::string blinn_phong_specular(
		...
		static std::string fs_instancing_partial(
		...
		static std::string fs_instancing_src = noob::concat(shader_prefix, lambert_diffuse, blinn_phong_specular, fs_instancing_partial);
