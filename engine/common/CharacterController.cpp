...
void noob::character_controller::init(btDynamicsWorld* _dynamics_world, noob::body* _rigid_body, float _timestep)
{
	...
	rigid_body->inner_body->setSleepingThresholds(0.0, 0.0);
	rigid_body->inner_body->setAngularFactor(0.0);
	dt = _timestep;
}
...
void noob::character_controller::update() 
{
	...
	rigid_body->inner_body->getMotionState()->getWorldTransform(xform);
...
void noob::character_controller::move(bool forward, bool backward, bool left, bool right, bool jump)
{
	if (on_ground())
	{
		...
		btVector3 linear_velocity = rigid_body->inner_body->getLinearVelocity();
		btScalar speed = rigid_body->inner_body->getLinearVelocity().length();
		...
		rigid_body->inner_body->setLinearVelocity(linear_velocity);
	}
}
